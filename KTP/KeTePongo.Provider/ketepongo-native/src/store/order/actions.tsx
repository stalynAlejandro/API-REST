  ...
  const payload : { currentOrder: ICurrentOrder } = {
    currentOrder: {
      ...
      orderLines: {
        ...
        [productId]: {
          ...currentOrder.orderLines[productId],
...
const removeProductFromOrderLines = ({ orderLines, product }: IRemoveOrderLineData) : ICurrentIndexOrder => {
  ...
  delete newOrderLines[product.id];
  return newOrderLines;
}

const removeProductFromProductLines = ({ productLines, index }: IRemoveProductLineData) : number[] => {
...
export const removeProductFromOrderRequest = (product: IProduct) => async (dispatch: Dispatch, getState: () => AppState) => {
  ...
  const index = orderLines[product.id].index;
...
export const reduceProductFromOrderRequest = (product: IProduct) => async (dispatch: Dispatch, getState: () => AppState) => {
  const currentOrder = getState().order.currentOrder;
  const { productLines, orderLines } = currentOrder
  const targetOrderLine = orderLines[product.id];
    ...
    payload = {
      currentOrder: {
        ...
        orderLines: {
          ...orderLines,
          [product.id] : {
            ...orderLines[product.id],
            quantity: orderLines[product.id].quantity - 1
          }
        }
      },
    }
...
export const addProductToOrderRequest = (product: IProduct) => async (dispatch: Dispatch, getState: () => AppState) => {
  ...
  if (targetOrderLine) {
    ...
    payload = {
      currentOrder: {
        ...currentOrder,
        productLines: [...productLines, product.id],
        orderLines: {
          ...orderLines,
          [product.id]: {
            product,
...
export const navigateBackToCatalog = () => (dispatch: Dispatch) => navigate(ROUTES.ProductCatalogScreen, dispatch);
