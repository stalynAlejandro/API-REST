...
const productCarteReducer : Reducer<IProductsCarteState,ICatalogProductAction> = (state = initialState, action: any) => {
  ...
  switch (action.type) {
    case RELOAD_PRODUCTSCARTE_DATA:
      return {
        ...state,
        ...action.payload as IProductCarte,
      };
      case ADD_PRODUCTCARTE_TO_CATALOG_SUCCEEDED:
        return {
          ...
          dictionary: {
            ...state.dictionary,
            [(action.payload as IProductCarte).id]: action.payload
          },
          loading: false,
          error: null
        };
        ...
        let list_updated = state.list.map(item => {
          if (item.id == (action.payload as IProductCarte).id) {
        ...
        return {
          ...
          dictionary: {
            ...state.dictionary,
            [(action.payload as IProductCarte).id]: action.payload as IProductCarte
          },
          loading: false,
          error: null
        };
