...
const BusinessRegistrationForm = ({ consumerType, code, serverErrorValidations }: { consumerType: number, code: any, serverErrorValidations: { [key: string]: string } }) => {
  ...
  const errorValidation = () => {
    ...
    if (methods.errors["tradeName"]) {
      (consumerType === ConsumerType.Professional) ? tradeNameRef.current.focus() : addressRef.current.focus();
      return;
    }

    if ((consumerType === ConsumerType.Professional) && addressRef.current && tradeNameRef.current.isFocused()) {
      return;
    }
  ...
  return (
    <View>
      {(consumerType === ConsumerType.Professional) &&
        <View>
          <Controller
            as={
              <ProviderCoverPhoto
      ...
      {(consumerType === ConsumerType.Individual) &&
        <View style={styles.body}>
          <View style={styles.individualConsumer}>
            <Text style={styles.titleIndividualConsumer}>Dirección de envío</Text>
            <Text style={styles.subtitleIndividualConsumer}>Aquí puedes establecer tu dirección para los locales que envíen a domicilio</Text>
          </View>


          <TypoGraphyOpenSansBold text="Dirección" style={styles.label_input} />
          <AddressInput
            placeholder={"Introduce aquí la dirección"}
            control={methods.control}
            name={"address"}
        ...
        {(consumerType === ConsumerType.Professional) ?
          <View>
            ...
            <Controller
              ...
              onSave={(data) => {
                methods.setValue("sanitaryMeasures", data)
              }}
            />
            <IdInput
              placeholder={"id"}
          ...
          :
          <View style={styles.spaceWhenNoIsProfessional}>
          </View>
        }
      </View>
    </View >
