...
import { connectToSignalR } from "../notifications/operations";
import { BusinessRegistrationHeadings } from "containers/Auth/AuthBusinessRegistrationScreen/AuthBusinessRegistrationScreen.UI";
...
export const registerBusiness = (newBusiness: IBusinessDataWithImage) => async (
  ...
  await simpleAsycnActionCreator(STRINGS.POST, "apiConsumers", 'Consumer',
    STRINGS.appJson,
    ...
    async () => {
      reloadAccessToken(true)(dispatch, getState);
...
export const updateBusiness = (business: IBusinessDataWithImage) => async (
  ...
  await simpleAsycnActionCreator(STRINGS.PUT, "apiConsumers", 'Consumer',
    STRINGS.appJson,
...
export const sendInvitations = (newInvitations: IInvitationsData) => async (
  ...
  if (registerRequest.length > 0) {
    await simpleAsycnActionCreator(STRINGS.POST, "apiConsumers", 'ConsumerInvitation',
      STRINGS.appJson,
...
export const ActivateAsConsumer = (dataLogin: ILoginData) => async (dispatch: Dispatch, getState) => {
  await simpleAsycnActionCreator(STRINGS.GET, "apiConsumers", "ConsumerActivation",
    STRINGS.appJson,
    "",
    null,
    actionConsts.REQUEST_TOKEN_FAILED,
  ...
  try {
    response = await db.apiAccessTokenWithoutAuthentication(headers).post(
      ...
      qs.stringify({
        ...
        client_id: "KeTePongoConsumerApp",
        client_secret: "-,ewñ89(%./jmlfSDggvml3,eoru",
...
export function checkAppVersionIsSupported(appVersion: string): Promise<AxiosResponse<string>> {
  return axios.create({
    headers: { "Content-Type": STRINGS.appJson, "client-id": "KeTePongoConsumerApp", "client-version": appVersion },
    baseURL: `${BASEURL}${BASEAPIMODULES.UsersAPI}`,
    timeout: 3500
